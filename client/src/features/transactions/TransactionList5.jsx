...
export default function TransactionList5() {
  const dispatch = useDispatch();
  const { list } = useSelector((state) => state.table5);
  ...
  const handleDrop = (e, dropIndex) => {
    ...
    const updated = newList.map((item, i) => ({
      ...item,
      sno: i + 1,
    }));

    dispatch(reorderLocal5(updated));
    ...
    return (
      <div className="empty-state">
        <div className="empty-icon">
          <svg width="32" height="32" fill="none" stroke="#6366f1" strokeWidth="1.5" viewBox="0 0 25 25">
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-3-3v6M5 6h16M5 18h16" />
          </svg>
        </div>
        <h3 style={{ fontSize: "1rem", fontWeight: 600, color: "#e2e8f0", marginBottom: "6px" }}>
          No transactions yet
        </h3>
        <p style={{ fontSize: "0.8rem", color: "#575569", marginBottom: "16px" }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div className="summary-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "12px" }}>
        ...
        <div className="stat-card stat-card-expense">
          <p style={{ fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: "#fda5af", marginBottom: "6px" }}>
            Expense
          </p>
        ...
        <div className="stat-card stat-card-balance">
          <p style={{ fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: "#a5b5fc", marginBottom: "6px" }}>
            Balance
          </p>
          <p style={{
            fontSize: "1.15rem",
      ...
      {/* Drag hint and Import Excel Button */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <p style={{ fontSize: "0.72rem", color: "#575569", display: "flex", alignItems: "center", gap: "6px", margin: "0" }}>
      ...
      {/* TABLE */}
      <div style={{ overflowX: "auto", borderRadius: "15px", border: "1px solid rgba(255,255,255,0.06)" }}>
        <table className="et-table">
          <thead>
            <tr>
              <th style={{ width: "32px", padding: "8px 5px" }} title="Drag to reorder">⠿</th>
              <th>S.no</th>
              <th>Description</th>
          ...
          <tbody>
            {(() => {
              ...
              return processedList.map((tx, index) => {
                ...
                const rowStyle = {
                  opacity: isDraggingThis && isDragging ? 0.5 : 1,
                ...
                if (isHeading && !isEditing) {
                  return (
                    <tr
                      ...
                      style={rowStyle}
                    >
                      <td style={{ background: "#FFFFAA", cursor: "grab", textAlign: "center", padding: "8px 5px", fontSize: "16px", color: "rgba(0,0,0,0.6)" }}>
                        ⠿
                      </td>
                      ...
                      <td style={{ background: "#FFFFAA", textAlign: "center" }}>
                        ...
                        <button className="et-delete-btn" onClick={() => dispatch(deleteTable5(tx._id))}>✕</button>
                      </td>
                    </tr>
                  );
                }
                      ...
                      <div style={{ display: "flex", gap: "6px", justifyContent: "center", alignItems: "center" }}>
                        {isEditing ? (
                          ...
                          <>
                            ...
                            <button className="et-delete-btn" onClick={() => dispatch(deleteTable5(tx._id))} title="Delete">✕</button>
                          </>
                        )}
                      </div>
                    </td>
