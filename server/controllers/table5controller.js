const Table5 = require("../models/Table5");

exports.getTransactions = async (req, res) => {
  try {
    const transactions = await Table5.find().sort({ createdAt: -1 });
...
exports.addTransaction = async (req, res) => {
  try {
    const newTransaction = new Table5(req.body);
...
exports.updateTransaction = async (req, res) => {
  try {
    const updated = await Table5.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.deleteTransaction = async (req, res) => {
  try {
    await Table5.findByIdAndDelete(req.params.id);
...
exports.reorderTransactions = async (req, res) => {
  try {
    ...
    await Promise.all(
      updates.map(({ id, sno }) =>
        Table5.findByIdAndUpdate(id, { sno }, { new: true })
      )
    );
...
exports.verifyImport = async (req, res) => {
  try {
    ...
    const existingMatches = await Table5.find({ $or: orConditions });
...
exports.importTransactions = async (req, res) => {
  try {
    ...
    let counter = await Counter.findById("table5_sno");
    ...
    const saved = await Table5.insertMany(docsToInsert);
